...
const DEFAULT_CONFIGS: Record<string, KalmanConfig> = {
  O2_PP: {
...
export class KalmanFilterService {
  ...
  private spikeDetectionCount: number = 0;
  private outlierRejectedCount: number = 0;
  ...
  filter(measurement: number, timestamp?: number): {
    ...
    if (this.innovationBuffer.length > 64) this.innovationBuffer.shift();
    ...
    if (this.spikeDetectionCount > 0 && this.spikeDetectionCount % 1000 === 0) {
      this.logger.warn(
        `卡尔曼滤波统计: 剔除离群=${this.outlierRejectedCount} 次, ` +
  ...
  private adaptiveProcessFactor(measurement: number): number {
    if (this.lastMeasurement === undefined) return 1;
  ...
  private lastMeasurementTimestamp?: number;

  private computeInnovationStats(): { mean: number; std: number } {
    const n = this.innovationBuffer.length;
    if (n === 0) return { mean: 0, std: 1 };
    let sum = 0;
    for (let i = 0; i < n; i++) sum += this.innovationBuffer[i];
    const mean = sum / n;
    let sqSum = 0;
    for (let i = 0; i < n; i++) {
  ...
  getEstimate(): number { return this.x; }
  getErrorCovariance(): number { return this.p; }
  getStats() {
    return {
