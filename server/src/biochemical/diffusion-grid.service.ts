...
import { BiochemicalState, GasDiffusionGrid, BIOCHEMICAL_CONSTANTS } from '../common/interfaces/biochemical.interface';

@Injectable()
export class DiffusionGridService implements OnModuleInit {
  private gridWidth = 64;
  private gridHeight = 64;
  private o2Grid: Float32Array;
  private co2Grid: Float32Array;
  private pressureGrid: Float32Array;
  private vX: Float32Array;
  private vY: Float32Array;
  private lastFrame: number = 0;
  private timeStep: number = 0;
  private cabinMask: Uint8Array;

  private readonly diffusionCoefO2 = 2.1e-5;
  private readonly diffusionCoefCO2 = 1.6e-5;
  ...
  onModuleInit() {
    ...
    this.logger.log(
      `流体扩散网格初始化: ${this.gridWidth}x${this.gridHeight} = ${this.gridWidth * this.gridHeight} 格点`,
    );
  }
  ...
  private initializeGrids() {
    const baseO2 = BIOCHEMICAL_CONSTANTS.NOMINAL_O2_FRACTION;
    const baseCO2 = BIOCHEMICAL_CONSTANTS.NOMINAL_CO2_PPM / 1_000_000;
    const baseP = BIOCHEMICAL_CONSTANTS.STANDARD_ATMOSPHERE_KPA;
  ...
  private injectBoundaryConditions(state: BiochemicalState) {
    ...
    for (const [sx, sy, strength] of sources) {
      ...
      if (!this.cabinMask[idx]) continue;
  ...
  private getSourceLocations(): Array<[number, number, number]> {
    return [
  ...
  private getSinkLocations(): Array<[number, number]> {
    return [
  ...
  private applyAdvection(dt: number) {
    const w = this.gridWidth;
    const h = this.gridHeight;
  ...
  private bilinear(src: Float32Array, i00: number, i10: number, i01: number, i11: number, fx: number, fy: number): number {
